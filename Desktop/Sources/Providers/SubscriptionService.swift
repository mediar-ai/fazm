...
final class SubscriptionService {
    ...
    /// With `trialDays == 0` there is no free trial: the paywall fires unless
    /// the user has an active Stripe subscription (which includes the
    /// post-checkout Stripe trial status `trialing`).
