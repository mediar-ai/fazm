...
// The Claude OAuth flow is hosted inline inside `PersonalAccountChooserSheet`
// (the three-model chooser). There is intentionally no standalone Claude auth
// window — every "connect an account" entry point routes through the chooser so
// the user always sees Gemini / Claude / ChatGPT together.
