...
# =============================================================================
RELEASE_LOG="/private/tmp/fazm-release.log"
...
# Configuration
BINARY_NAME="Fazm"  # Package.swift target — binary paths, lipo, CFBundleExecutable
APP_NAME="Fazm"
...
FFMPEG_PATH="$APP_BUNDLE/Contents/Resources/Fazm_Fazm.bundle/ffmpeg"
...
if [ -f "fazm_icon.icns" ]; then
    ...
    if command -v fileicon &> /dev/null; then
        fileicon set "$DMG_PATH" fazm_icon.icns && echo "    Icon set successfully" || echo "    Warning: Could not set icon"
    else
...
if command -v gh &> /dev/null; then
    ...
    # Create GitHub release with both Fazm.zip and DMG
    gh release create "$RELEASE_TAG" \
        ...
        echo "  ✓ Uploaded: Fazm.zip (for auto-update)"
